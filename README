Run ```./configscript.sh``` first.
Then, build the project (e.g., run ```make -j4```)
You will get a bunch of *.dot files under ```src/```.
Use dot opener (e.g., ```xdot``` to see the graph).


===== BELOW IS ABOUT COREUTILS =====
